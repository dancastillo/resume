...
.config(['commandBrokerProvider', function (commandBrokerProvider) {
    ...
    commandBrokerProvider.appendCommandHandler({
        command: 'github',
        description: ["Visit Dan Castillo's Github page."],
        handle: function (session) {
            session.output.push({ output: true, text: [window.open('http://github.com/dancastillo', '', 'width=1000')], breakLine: true });
        }
    });

    commandBrokerProvider.appendCommandHandler({
        command: 'linkedin',
        description: ["Visit Dan Castillo's LinkedIn page."],
        handle: function (session) {
            session.output.push({ output: true, text: [window.open('http://linkedin.com/in/daniel-castillo', '', 'width=1000')], breakLine: true });
        }
    });

    commandBrokerProvider.appendCommandHandler({
        command: 'resume',
        description: ["To download Dan Castillo's resume."],
        handle: function (session) {
            session.output.push({ output: true, text: ['Working on downloading resume'], breakLine: true });
        }
    });
var emailSend = "mailto:dancastillo99@gmail.com";
    commandBrokerProvider.appendCommandHandler({
        command: 'email',
        description: ["To display Dan Castillo's email address."],
        handle: function (session) {
            session.output.push({ output: true, text: [window.location.href = emailSend], breakLine: true });
        }
    });

    commandBrokerProvider.appendCommandHandler({
        command: 'projects',
        description: ["Find out about some of my relevant projects!"],
        handle: function (session) {
            session.output.push({ output: true, text: ['Android Application - Report-It \n  Handles the creation, modification, sharing, and deletion of on-site reports.\n\n  Binary Search Tree Operatons \n  Shows nodes that were added, deleted, and seaches for nodes also shows in-order, pre-order, and post-order.\n\n  Visit my Github page for more projects!'], breakLine: true });
        }
    });

    commandBrokerProvider.appendCommandHandler({
        command: 'education',
        description: ["Find out where I went to school!"],
        handle: function (session) {
            session.output.push({ output: true, text: ['Southern Connecticut State University \n  Bachelors of Science in Computer Science  -  May 2016 \n\n  Norwalk Community College \n  Associates of Science in Computer Science  -   December 2014'], breakLine: true });
        }
    });

    commandBrokerProvider.appendCommandHandler({
        command: 'work',
        description: ["Find out my work history!"],
        handle: function (session) {
            session.output.push({ output: true, text: ['Independent Software A100 Program   ---   06/2016 - Present \n\n  Innovative Engineering Services, LLC   ---   IT Technician'], breakLine: true });
        }
    });
/*
    commandBrokerProvider.appendCommandHandler({
    ...
    var helpCommandHandler = function () {
        ...
        me.handle = function (session, cmd) {
            ...
            if (cmd) {
                ...
                //for (var i = 0; i < list.length; i++) {
                //to only display some function I added
                for (var i = 0; i < 8; i++) {
